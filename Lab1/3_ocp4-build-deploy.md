...
**注意: ここから先は全て一般ユーザーで操作を行います。前回のLabで管理者権限になっている場合は、一旦ログアウトして再度ご自身のユーザーで入り直してください**
...
1. プロジェクト名(例: `blog-dev00` )を指定し，**Create** を選択します。  
  
    プロジェクト名には，**必ずご自身のログイン時のユーザー名 (例: "blog-dev00")** を指定してください。  
...
1. アプリケーションのリポジトリなどを指定して，OpenShift4上にアプリケーションをデプロイします。
    ...
    - Namespace: `自身のプロジェクト名 (例: blog-dev00)`
    - Version: `python:3.6`
    - Name:`任意の名前(例: blog-dev00)`
...
1. [Workloads] > [Pods] > [指定したアプリ名 (例: blog-dev00)] のように選択します。
...
### 3-3-5. アプリケーションの動作確認
1. [Networking] > [Routes] を選択し，blog用のRouter(例: `blog-dev00`)行にある **Location欄のリンク** を開きます。
...
- Project名(NameSpace): `trial-<yourID>` (例: `trial-dev00`)
